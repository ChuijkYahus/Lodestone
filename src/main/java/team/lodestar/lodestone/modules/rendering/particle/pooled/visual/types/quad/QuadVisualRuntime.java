...
public class QuadVisualRuntime implements ParticleVisualRuntime {
    private final QuadVisualConfig config;
    private static VertexBuffer vertexBuffer;

    public QuadVisualRuntime(QuadVisualConfig config) {
        this.config = config;
    }
    ...
    public void collect(ParticleVisualCollectContext context, ParticlePool pool, int liveCount) {
        ...
        if (vertexBuffer == null) {
            LodestoneRenderSystem.wrap(QuadVisualRuntime::initVBO);
        }
        var drawData = new QuadVisualDrawData(context.particles(), liveCount, context.targetVisualId());
        var submission = new ParticleVisualSubmission(config.renderType, config.instanceFormat, vertexBuffer, drawData, QuadBatchRenderer.INSTANCE);
